...
<iso:schema xmlns:iso="http://purl.oclc.org/dsdl/schematron" xmlns:tei="http://www.tei-c.org/ns/1.0"
	...
	<!--  	
		...
		tei_test_final.xsl  uncp-ISBN-book.xml > results.xml
		iso_html.xsl  results.xml > brief-results.html
	-->
	
	<!--
		For readable path notation, set the default in iso_svrl_for_xslt2.xsl to '2'
		check iso_schematron_skeleton_for_saxon.xsl for documentation.
	-->
	...
	<iso:let name="isbn" value="substring-before($truncate,'.')"/>
	<iso:let name="path" value="substring-before($filename,$truncate)"/>

	<!-- the following a1 and a2 depend upon filename structure of uncp-isbn-book -->
	...
	<iso:pattern id="PrimaryID">
		<iso:rule context="//tei:TEI">
			...
			<iso:assert test="$isbn castable as xs:integer" role="Priority1" id="a2"
					>The ISBN should have 13 digits: <iso:value-of select="$isbn"/></iso:assert>
			<iso:assert test="$isbn=substring(@xml:id,6,13)" role="Priority1" id="a111"
				>The ISBN in the @xml:id should match filename: <iso:value-of select="substring(@xml:id,6,13)"/> does not match <iso:value-of select="$isbn"/></iso:assert>
	...
	<iso:pattern id="pb.checks">
		...
		<iso:rule context="//tei:pb">
			...
			<iso:assert test="(@xml:id)=concat('pg',@n)" role="Priority1" id="a5"
					>The pb id should have the following structure: pg{pageNo}: <iso:value-of select="@xml:id"
				/>.</iso:assert>
			<iso:assert test="substring-after(@xml:id,'pg')=(@n)" role="Priority1" id="a6"
					>The id page number should be the same as the n page number: check the id (<iso:value-of
					select="@xml:id"/>) against n (<iso:value-of select="@n"/>).</iso:assert>
...
<!--	<iso:pattern id="body.checks">
	...
	<!-- Check idno for @type='local' -->
	<iso:pattern id="id_pub.local.checks">
		<iso:title>Checking the ID for proper type</iso:title>
		<iso:p>ISBN ID check</iso:p>
		<iso:rule context="//tei:idno">
			<iso:report test="@type='local'" role="Priority1" id="a522">idno @type should be ISBN, not local.</iso:report>
		</iso:rule>
	</iso:pattern>
	...
	<iso:pattern id="div.checks">
		...
		<iso:rule context="//tei:div">
			<iso:assert test="tei:head" role="Priority2" id="a117"
				>All divs generally have a head</iso:assert>
			<iso:assert test="(@type)" role="Priority1" id="a118"
				>All divs must have a type or type attribute</iso:assert>
			<iso:report test="(@subtype)" role="Priority1" id="r119"
				>Subhead sections should have a type attribute instead of subtype</iso:report>
		</iso:rule>
	</iso:pattern>
	...
	<iso:pattern id="div_head.checks">
		...
		<iso:rule context="//tei:div[@type='ahead']">
			<iso:assert
				...
				role="Priority2" id="a19"
					>ahead sections are generally children of chapters: this div has a type of <iso:value-of
	...
	<!-- Check that lists have type attribute -->
	<iso:pattern id="list-type.checks">
		<iso:title>Checking the list Structure</iso:title>
		<iso:p>Checking list for @type</iso:p>
		<iso:rule context="//tei:list">
			<iso:assert test="(@type)" role="Priority1" id="a342"
				>All lists should have a type attribute</iso:assert>
		</iso:rule>
	</iso:pattern>
